...
void main()
{
	out_Color=texture(opaqueSampler,out_vs_texCoords).rgba;
	...
	Color=texture(texSampler1,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(0,1,0),clamp(out_Color.a+(Color.a*0.125),0,1));

	if(renderLayer==2)
		return;

	Color=texture(texSampler2,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(0,0,1),clamp(out_Color.a+(Color.a*0.0625),0,1));

	if(renderLayer==3)
		return;

	Color=texture(texSampler3,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(1,1,0),clamp(out_Color.a+(Color.a*0.03125),0,1));

	if(renderLayer==4)
		return;

	Color=texture(texSampler4,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(0,1,1),clamp(out_Color.a+(Color.a*0.015625),0,1));

	if(renderLayer==5)
		return;

	Color=texture(texSampler5,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(1,0,1),clamp(out_Color.a+(Color.a*0.0078125),0,1));

	if(renderLayer==6)
		return;

	Color=texture(texSampler6,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(1,1,1),clamp(out_Color.a+(Color.a*0.00390625),0,1));

	if(renderLayer==7)
		return;

	Color=texture(texSampler7,out_vs_texCoords).rgba;
	out_Color=vec4((1-Color.a)*out_Color.rgb + Color.a*vec3(0,0,0),clamp(out_Color.a+(Color.a*0.001953125),0,1));*/
